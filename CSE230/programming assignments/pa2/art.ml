...
*)
(*
  In this file, we impletment two funcitons build and build2
  As the instructions says, there will be a rand function and a depth as input.
  The rand(0,a)function will return a random integer within the interval [0,a).
  And the depth is the recursive depth. If the depth is 0, we can consider it as the base. we will produce the most basic expression VarX or VarY.
  Else we will randomly select the defined operations in expr.ml
*)
...
(*
  build2 is an extensive function of function build, which contains two new operations that we defined before.
*)
let rec build2 (rand,depth) = 
   ...
   match rand (0, 7) with
      ...
      | 4 -> buildThresh(build(rand, depth -1), build(rand, depth - 1), 
             build(rand, depth -1),build(rand, depth -1))
      | 5 -> buildSigmoid(build(rand, depth -1))
      | 6 -> buildSineTripple(build(rand, depth -1),build(rand, depth -1),build(rand, depth -1))
      | _ -> buildSine(build(rand, depth - 1))
